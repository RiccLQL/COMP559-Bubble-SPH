let Engine = (function() {
    ...
    function Grid() {
        ...
        function Cell() {
            ...
            this.lightParticles = new Array(INIT_MAX_PARTICLES_IN_CELL);
            this.halfNeighbors = new Array(0);
            this.numParticles = 0;
            this.numLightParticles = 0;
        }
        ...
        */
        ...
        this.hardresetLight = function() {
            for (let c of this.cells) {
                c.numLightParticles = 0;
                c.lightParticles = new Array(INIT_MAX_PARTICLES_IN_CELL);
            }
        }
        ...
        this.addLightParticleToCell = function(p) {
            let c = this.getCellFromLocation(p.x, p.y);
            if (c != null) {
                c.lightParticles[c.numLightParticles++] = p;
            }
            else
                console.log("Undefined grid cell!");
        }
    ...
    function AddPressureForce(p1, p2) {
        let r2 = dist2(p1, p2);
        if (r2 < h2) {
    ...
    function AddViscosityForce(p1, p2) {
        let r2 = dist2(p1, p2);
        if (r2 < h2) {
    ...
    function AddCohesionForces(p1, p2) {
        // cohesion
        let sumCohX = 0;
        let sumCohY = 0;
        // for (let neighbor of cell.halfNeighbors) {
            // for (let i = 0; i < neighbor.numParticles; i++) {
            //     let p2 = neighbor.particles[i];
            //     let r2 = dist2(p1, p2);
            //     let r = Math.sqrt(r2) + 1e-6;
            //     sumCohX += p2.rho * (p1.x - p2.x);
            //     sumCohX += p2.rho * (p1.y - p2.y);
            //     numNeighbors++;
            // }
    ...
    function AddDragForces(p1, p2, isAir) {

        // numNeighbors++;
            // }
        // }
        const kd = isAir ? kdAir : kdWater;
        // drag
        let sumDragX = 0;
        let sumDragY = 0;
        // for (let neighbor of cell.halfNeighbors) {
        ...
        let r2 = dist2(p1, p2);
        let r = Math.sqrt(r2) + 1e-6;
        let dotProd = ((p1.Vx - p2.Vx) * (p1.x - p2.x)) + ((p1.Vy - p2.Vy) * (p1.y - p2.y));
        let projX = (dotProd <= 0 || ((p1.x - p2.x) + h2) === 0) ? 0 : (dotProd / ((p1.x - p2.x) + h2));
        let projY = (dotProd <= 0 || ((p1.y - p2.y) + h2) === 0) ? 0 : (dotProd / ((p1.y - p2.y) + h2));
        sumDragX += p2.m * ((kd * h * 0.002) / (p1.rho + p2.rho)) * projX * Wvisc_lapl(p1.x - p2.x);
        sumDragY += p2.m * ((kd * h * 0.002) / (p1.rho + p2.rho)) * projY * Wvisc_lapl(p1.y - p2.y);
            // }
            ...
            //     let projX = (dotProd <= 0 || ((p1.x - p2.x) + h2) === 0) ? 0 : (dotProd / ((p1.x - p2.x) + h2));
            //     let projY = (dotProd <= 0 || ((p1.y - p2.y) + h2) === 0) ? 0 : (dotProd / ((p1.y - p2.y) + h2));
            //     sumDragX += p2.m * ((kdAir * h) / (p1.rho + p2.rho)) * projX * Wvisc_lapl(r);
            //     sumDragY += p2.m * ((kdAir * h) / (p1.rho + p2.rho)) * projY * Wvisc_lapl(r);
            // }
        // }
        let DragFx = p1.m * sumDragX;
        let DragFy = p1.m * sumDragY;
    ...
    function CalcForcedVelocity() {
        ...
        for (let i = 0; i < forceVelocityCell.numLightParticles; i++) {
            let p = forceVelocityCell.lightParticles[i];
            p.Vx = forceVx;
            p.Vy = forceVy;
            p.Fx = 0;
            p.Fy = 0;
    ...
    return {
        ...
        setLightParticles: function(n) {
            let i0 = 0;
            if (lightParticles == undefined) {
                lightParticles = new Array(n);
            }
            else {
                if (n < lightParticles.length) {
                    grid.hardresetLight();
                    lightParticles.length = n;
                }
                i0 = lightParticles.length;
            }
            for (let i = i0; i < n; i++) {
                lightParticles[i] = new Particle();
                lightParticles[i].m = mAir;
                lightParticles[i].rho = lightParticles[i].m * Wpoly6(0);
            }
        },
        ...
        getLightParticlePosition: function(i, position) {
