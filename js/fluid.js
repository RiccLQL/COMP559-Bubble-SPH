...
let lightmat, lightcircle, lightmeshes;
...
function setLightParticles() {
    ...
    if (lightmeshes == undefined) {
        ...
        if (n < lightmeshes.length) {
            for (var i = n; i < lightmeshes.length; i++) {
                scene.remove(lightmeshes[i]);
            }
            lightmeshes.length = n;
        }
        i0 = lightmeshes.length;
    }

    engine.setLightParticles(n);

    for (var i = i0; i < n; i++) {
        var m = new THREE.Mesh(lightcircle, lightmat);
        lightmeshes[i] = m;
        scene.add(m);
    }
}
...
function doLoop() {
    ...
    for (let i = 0; i < lightmeshes.length; i++) {
        engine.getLightParticlePosition(i, lightmeshes[i].position)
    }
    renderer.render(scene, camera);
